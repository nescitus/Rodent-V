...
var suitThresholds = []int{
	-75, 0, 0, 0, 0, 100, 0, -500, -500, 225, 
	225, 900, 225, -100, 0, 0, 0, 100, 100, 0, 
	100, 500, 325, 325, -225, -400, 100, -175, 150, -225, 
	100, 0, 0, 0, 325, -225, 100, 100, 0, -900, 
	-225, -400, 100, 0, -225, -900, 100, 0, 100, -225, 
	0, -325, 0, -325, 900, -100, 225, -100, 225, 225, 
	0, 0, 0, 225, 325, 0, 725, 325, 0, 100, 
	0, 
}

var suitMoves = []string{
...
func TestSEE(t *testing.T) {
	...
	for i := 0; i < 71; i++ {
		p := new(Pos)
		parseFEN(p, suitFens[i])
		move := parseMove(p, suitMoves[i])
		threshold := suitThresholds[i]
		
		res := see(p, move, threshold)
		// We expect see(p, move, threshold) to return true for all tests because the array tests are designed to be true at these thresholds for Rodent-V's piece values
		if !res {
			t.Errorf("Test %d failed! FEN: %s | Move: %s | Threshold: %d", i, suitFens[i], suitMoves[i], threshold)
		} else {
			passCount++
		}
	}
	
	fmt.Printf("Passed %d / 71 SEE tests.\n", passCount)
}
